...
cmd_search() {
    ...
    local query_params
    query_params="pageSize=100&filter=$(printf '%s' "$filter" | jq -sRr @uri)"
...
cmd_rename() {
    ...
    local query_params
    query_params="pageSize=1000&filter=$(printf '%s' "$filter" | jq -sRr @uri)"
    local memos_json
    ...
    while IFS= read -r memo_id; do
        ...
        local new_content
        new_content=$(printf '%s' "$content" | sed "s/#${old_tag}\\b/#${new_tag}/g")
